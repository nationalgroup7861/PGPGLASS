...
const PgpGptContext = createContext();
PgpGptProvider.propTypes = {
  children: PropTypes.node,
};
function PgpGptProvider({ children }) {
  ...
  async function GetChatsessionList() {
    setIsLoading(true)
    try {
      const params = { client_id: userInfo.id, type: "gpt" ,search:searchKeyword };
  ...
  async function GetPermissionandkey() {
    ...
    try {
      ...
      if (response) {
        ...
        setChatGptKey(data?.chat_gpt3_key)
...
export { PgpGptContext, PgpGptProvider };
